...
def push_log(log_val):
    ...
    stp_log.write(f"[{curr_time}]: {log_val}\n")
    stp_log.close()

#Create Log, Json, Env Files
log_dir = os.path.abspath(os.getcwd()) + "\\" + "Log_Push"
...
for i in ["Log_Push/log_stp.txt", "Log_Push/log_launch.txt", ".env", "val.json"]:
    try:
        try:
            ...
            push_log(f"File (re)generation err -> \"{re.search(r'[^/]+$', i).group(0)}\" -> {err}")
...
def stp_vals():
    print("These are examples for \"Opera Gx\" run, you can change the parameters:\n⌄ ⌄ ⌄  ⌄ ⌄ ⌄  ⌄ ⌄ ⌄  ⌄ ⌄ ⌄  ⌄ ⌄ ⌄  ⌄ ⌄ ⌄  ⌄ ⌄ ⌄")
    for i in val:
        try:
            if i=="brw_exe_path":
                print("*- \"If you press \"Enter\", it will automatically search for the browser path in the drive/path you specified.\"\n"
                      "*- \"Please wait for the message to show up, as it may take some time to complete the search...\"\n⌄ ⌄ ⌄  ⌄ ⌄ ⌄  ⌄ ⌄ ⌄")
            var = input(f"— Input {i} value [Default example value: {val[i]}]\nIf you don't want to change it, then press \"Enter\" <<  ")
            if var.strip():
                temp_val[i]=val[i]=var.strip().lower()
                print(f"-\"{var.strip().lower()}\"")
            else: print(f"-\"{val[i]}\"")
            if i == "brw_exe_path":
                global bw_path
                if not var:
                ...
                print(f"\r \b*- Search is Done! -> result: {val[i]}")
